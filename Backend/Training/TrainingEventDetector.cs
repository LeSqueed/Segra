...
public class TrainingEventDetector : IDisposable
{
    ...
    public event Action<List<TrainingEventDetectionResult>>? DetectionsAvailable;
    ...
    private async Task DetectionLoopAsync(CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                try
                {
                    var allResults = new List<TrainingEventDetectionResult>();
    ...
    internal static void MapDetectionsToFullFrame(List<TrainingEventDetectionResult> detections,
    ...
    internal static List<RegionGroup> BuildRegionGroups(List<TrainingEventDefinition> definitions)
    {
    ...
    private static List<TrainingEventDetectionResult> RunInferenceOnGray(
    ...
    private static List<TrainingEventDetectionResult> ParseYoloOutput(
        ReadOnlySpan<float> output, int inputSize)
    {
        var results = new List<TrainingEventDetectionResult>();
        ...
        for (int i = 0; i < numDetections; i++)
        {
            ...
            results.Add(new TrainingEventDetectionResult
            {
                ClassId = classId,
                EventId = classId,
                Confidence = maxConf,
